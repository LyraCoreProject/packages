...
Spawn and level-up arm a class-and-role profile. They do not grant anything. The next admitted
`Cohort` runner pass reconciles one missing entry through the core Actor API. Survival, usable
recovery, and defense keep their higher priority. `Legacy`, `RecordOnly`, `Frozen`, a current Account
owner, and a current World Session block every provisioning gameplay write.

Profiles make training and their bounded item grants free. They cover the configured kit, one class
